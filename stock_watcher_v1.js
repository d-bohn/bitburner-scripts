...
export async function main(ns) {
    ...
    if (args.help) {
        ns.tprint(`USAGE: run ${ns.getScriptName()} --liq <cash_to_keep_on_hand>`);
        ns.tprint(`  --liq: Amount of starting cash to keep liquid, either decimal percent or actual (default: 0.15)`);
        ns.tprint("Example:");
        ns.tprint(`> run ${ns.getScriptName()} --liq 0.15`);
        ns.tprint(`> run ${ns.getScriptName()} --liq 5000000`);
        return;
    }
    ...
    const percent_liquid = args.liq;
    const starting_cash = ns.getPlayer().money;

    let cash_to_keep; // Either a percent or a fixed number
    if (percent_liquid < 1) {
        cash_to_keep = starting_cash * percent_liquid;
    } else {
        cash_to_keep = starting_cash - percent_liquid;
    }

    ns.print(`Cash to keep: ${ns.nFormat(cash_to_keep, "$0.000a")}`);

    while (true) {
        tendStocks(ns, cash_to_keep);
        await ns.sleep(1 * 60 * 1000);
    }
}

function tendStocks(ns, cash_to_keep) {
    ...
    buyStocks(ns, stocksToBuy, 4, cash_to_keep);
    ...
    for (const stock of stocksToSell) {
        if (stock.shares > 0) {
            ...
            if (salePrice != 0) {
                ...
                stock.shares = 0;
                ns.toast(`Sold ${stock.summary} stock for ${ns.nFormat(saleProfit, "$0.0a")} profit`);
...
function buyStocks(ns, stocksToBuy, maxTransactions, cash_to_keep) {
    ...
    for (const stock of bestStocks) {
        ...
        // don't spend the X monies
        if (moneyRemaining < cash_to_keep || transactions >= maxTransactions) {
            return;
        }
